...
char  DPB[] = {
  isc_dpb_version1, isc_dpb_user_name, 6, 's', 'y', 's', 'd', 'b', 'a', isc_dpb_password, 9, 'm', 'a', 's', 't', 'e', 'r', 'k', 'e', 'y', isc_dpb_lc_ctype, 11, 'U', 'N', 'I', 'C', 'O', 'D', 'E', '_', 'F', 'S', 'S'
};
...
class ThreadTester {
  private:
    ...
    void          printStatus(ISC_STATUS_ARRAY Status, long Line = 0);
    void          printSqlError(ISC_STATUS_ARRAY Status);
    void          execute();
  public:
                  ThreadTester();
...
void ThreadTester::printStatus(ISC_STATUS_ARRAY Status, long Line)
{
  if( Status[1] != 0 ){
    isc_print_status(Status);
    if( Line > 0 )
...
void ThreadTester::execute()
{
  ...
  static char InsertString[]  = "INSERT INTO table1 (dep_id,dep_array) VALUES (GEN_ID(TABLE1_GEN,1),?)"
  ;
  ...
  isc_array_lookup_bounds(Status_, &DbHandle_, &TrHandle_, Sqlda.sqlvar[0].relname, Sqlda.sqlvar[0].sqlname, &ArrayDesc);
  printStatus(Status_, __LINE__);
  ...
  for( long i = 1000; i > 0; i-- ){
    ...
    __int64   Data[6]     = {
      10000000000, 10000000001, 10000000010, 10000000011, 10000000020, 10000000021
    };

    ISC_LONG  SizeOfData  = sizeof(Data);
    isc_array_put_slice(Status_, &DbHandle_, &TrHandle_, &ArrayId, &ArrayDesc, Data, &SizeOfData);
    printStatus(Status_, __LINE__);
...
Tester::Tester()
  : DbHandle_(0),
    TrHandle_(0)
{
  static char CreateDbString[]  = "CREATE DATABASE '" DbName "'" " USER 'sysdba' PASSWORD 'masterkey' PAGE_SIZE=1024" " DEFAULT CHARACTER SET UNICODE_FSS"
  ;

  if( isc_dsql_execute_immediate(Status_, &DbHandle_, &TrHandle_, 0, CreateDbString, 3, NULL) != 0 )
    if( isc_sqlcode(Status_) != -902 )
      printSqlError(Status_);
  ...
  static char CreateTableString[] = "RECREATE TABLE table1 (" " dep_id        INTEGER NOT NULL PRIMARY KEY," " dep_array     DECIMAL(18,0) [0:2,0:1]" ")"
  ;
  ...
  if( isc_dsql_execute_immediate(Status_, &DbHandle_, &TrHandle_, 0, DropGenString, 3, NULL) != 0 )
    if( Status_[1] != isc_no_meta_update )
      printStatus(Status_);
