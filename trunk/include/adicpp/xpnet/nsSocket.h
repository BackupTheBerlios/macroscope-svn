...
//----------------------------------------------------------------------
    ...
    int Send(unsigned char *aBuf, size_t *aBufSize);
    int Recv(unsigned char *aBuf, size_t *aBufSize);
    int Recv(unsigned char *aBuf, size_t *aBufSize, int aTimeoutThresholdUsecs);
    int Close();
...
private:
    ...
    char  *mHost;
    int    mPort;
    SOCKET mFd; // connected socket
    SOCKET mListenFd; // listening socket (only if SrvOpen() was called)
