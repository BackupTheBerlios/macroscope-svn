...
#include <adicpp/xpnet/nsSocket.h>
...
int 
nsSocket::Send(unsigned char *aBuf, size_t *aBufSize)
{
    intptr_t rv = OK;
    ...
    while (timeout < kTimeoutThresholdUsecs)
    {
        ...
        rv = select(1, NULL, &selset, NULL, &seltime);
        switch (rv)
        {
...
#if _MSC_VER
    rv = write(mFd, aBuf, (int) *aBufSize);
#else
...
int
nsSocket::Recv(unsigned char *aBuf, size_t *aBufSize)
{
  return(Recv(aBuf, aBufSize, kRecvTimeoutThresholdUsecs));
}

int
nsSocket::Recv(unsigned char *aBuf, size_t *aBufSize, int aTimeoutThresholdUsecs)
{
    intptr_t rv = OK;
    ...
    while (timeout < aTimeoutThresholdUsecs)
    {
        ...
        rv = select(1, &selset, NULL, NULL, &seltime);
        switch (rv)
        {
            case -1:            /* error occured! */
                return errno;
