...
nsHTTPConn::ParseContentLength(const char *aBuf, int *aLength)
{
    ...
    *aLength = 0;
    ...
    if (clHdr)
    {
        ...
        if (pos < eol)
        {
            size_t clValStrLen = eol - pos + 1; // extra byte to NULL terminate
...
intptr_t
nsHTTPConn::Base64Encode(const unsigned char *in_str, size_t in_len, 
