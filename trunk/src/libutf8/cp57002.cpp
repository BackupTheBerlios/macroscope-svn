//---------------------------------------------------------------------------
#include <adicpp/lconfig.h>
//---------------------------------------------------------------------------
namespace utf8 {
//---------------------------------------------------------------------------
#if defined(EMBED_CP57002_SUPPORT) || defined(EMBED_ALL_CP)
extern const unsigned char cp57002s2utf8s[114] = {
  0xAA, 0xDE, 0x00, 0x00, 0x0B, 0x00, 0x9F, 0x00,
  0x00, 0xA1, 0xA3, 0x01, 0x09, 0xA4, 0xAA, 0x05,
  0x09, 0xAB, 0xAD, 0x0E, 0x09, 0xAF, 0xB1, 0x12,
  0x09, 0xB3, 0xCD, 0x15, 0x09, 0xCF, 0xD8, 0x30,
  0x09, 0xDA, 0xDF, 0x3E, 0x09, 0xE0, 0xE2, 0x46,
  0x09, 0xE4, 0xE6, 0x4A, 0x09, 0xF1, 0xFA, 0x66,
  0x09, 0x15, 0xA0, 0x3F, 0x00, 0xAE, 0x0D, 0x09,
  0xB2, 0x11, 0x09, 0xCE, 0x5F, 0x09, 0xD9, 0x3F,
  0x00, 0xE3, 0x45, 0x09, 0xE7, 0x49, 0x09, 0xE8,
  0x4D, 0x09, 0xE9, 0x3C, 0x09, 0xEA, 0x64, 0x09,
  0xEB, 0x3F, 0x00, 0xEC, 0x3F, 0x00, 0xED, 0x3F,
  0x00, 0xEE, 0x3F, 0x00, 0xEF, 0x3F, 0x00, 0xF0,
  0x3F, 0x00, 0xFB, 0x3F, 0x00, 0xFC, 0x3F, 0x00,
  0xFD, 0x3F, 0x00, 0xFE, 0x3F, 0x00, 0xFF, 0x3F,
  0x00, 0x00
};
//---------------------------------------------------------------------------
extern const unsigned char utf8s2cp57002s[3168] = {
  
};
#endif
//---------------------------------------------------------------------------
} // namespace utf8
//---------------------------------------------------------------------------
